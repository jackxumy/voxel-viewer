...
function App() {
  ...
  const collisionEnabledRef = useRef<boolean>(true);
  ...
  useEffect(() => {
    ...
    // Wireframe materials for voxel edges (cheap line-like overlay)
    const wireMaterials: { [key: number]: MeshBasicMaterial } = {
      0: new MeshBasicMaterial({ color: 0x000000, wireframe: true }),
      1: new MeshBasicMaterial({ color: 0x000000, wireframe: true }),
      2: new MeshBasicMaterial({ color: 0x000000, wireframe: true }),
      3: new MeshBasicMaterial({ color: 0x000000, wireframe: true }),
    };
    ...
    async function loadChunkMesh(level: string, chunkId: string, meta: ChunkMeta, voxelSize: number) {
      ...
      try {
        ...
        // Create a wireframe instanced mesh to render voxel cube edges
        const wireMat = wireMaterials[level as unknown as keyof typeof wireMaterials] || wireMaterials[0];
        const wire = new InstancedMesh(geometry, wireMat, count);
        ...
        wire.visible = wireframeEnabledRef.current;
        // Attach wireframe as a child so visibility toggles with the solid mesh
        mesh.add(wire);
        // keep a handle for toggling later
        (mesh as any).userData.wire = wire;
    ...
    function applyWireframeVisibility(enabled: boolean) {
      for (const mesh of loadedChunksRef.current.values()) {
        const wire = (mesh as any).userData?.wire as InstancedMesh | undefined;
    ...
    const onToggleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Control') {
        ...
        collisionEnabledRef.current = !collisionEnabledRef.current;
      }
    };
    window.addEventListener('keydown', onToggleKeyDown);

    // --- Collision Detection ---
    function checkCollision(newPosition: Vector3, radius: number = 0.1): boolean {
    ...
    // --- Controls & Loop ---
    // wrapper that respects collision toggle
    const moveWithToggle = (currentPos: Vector3, movement: Vector3, radius: number = 0.25) => {
      if (!collisionEnabledRef.current) return currentPos.clone().add(movement);
      return moveWithSliding(currentPos, movement, radius);
    };
    const { processInput, cleanup: cleanupControls } = setupControls(camera, mount, moveWithToggle);
    ...
    return () => {
      ...
      Object.values(wireMaterials).forEach(m => m.dispose());
    };
  }, []);

...
function App() {
  ...
  const loadedChunksRef = useRef<Map<string, InstancedMesh>>(new Map());// 缓存已经载入的区块三角网
  const loadingQueueRef = useRef<Set<string>>(new Set());// 记录正在加载的区块
  ...
  useEffect(() => {
    ...
    async function loadChunkMesh(level: string, chunkId: string, meta: ChunkMeta, voxelSize: number) {
      ...
      try {
        ...
        const count = positions.length / 3;  // 实例个数

        // 创建 InstancedMesh
        const mat = materials[level as unknown as keyof typeof materials] || materials[0]; // 材质
        ...
        for (let i = 0; i < count; i++) {
          ...
          dummy.position.set(x, y, z); // 设置平移矩阵
          dummy.scale.set(scale, scale, scale);//  设置缩放矩阵
          dummy.updateMatrix(); // 计算最终的变换矩阵

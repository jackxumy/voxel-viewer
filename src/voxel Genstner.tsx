...
function App() {
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            // 4. 加载体素数据
            const voxelData = await loadVoxelBin('/platform_1024x1024x1.bin');
            ...
            // 7. 加载着色器
            const shaderCode = await fetch('/src/shader/voxel_Genstner.wgsl').then(r => r.text())
            const shaderModule = device.createShaderModule({ code: shaderCode });

            // 7.1 加载线框着色器
            const wireShaderCode = await fetch('/src/shader/vlines_Genstner.wgsl').then(r => r.text());
